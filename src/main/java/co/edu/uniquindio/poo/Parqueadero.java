...
public class Parqueadero {
    ...
    public Parqueadero(int filas, int columnas) {
        Parqueadero.puestos = new Puesto[filas][columnas];

        // Inicializar todos los puestos
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                puestos[i][j] = new Puesto(false, null);
            }
        }
    }
    ...
    public static void ubicarVehiculo(int fila, int columna, Vehiculo vehiculo) {
        if (verificarDisponibilidad(puestos, fila, columna)) {
            puestos[fila][columna].ocupar(true, vehiculo);
            System.out.println(
                    "Vehículo ubicado en el puesto [" + fila + "][" + columna + "] a las " + LocalDateTime.now() + ".");
            System.out.println();
        } else {
        ...
        for (int i = 0; i < puestos.length; i++) {
            for (int j = 0; j < puestos[i].length; j++) {
                if (puestos[i][j].isEstaOcupado()) {
                    ...
                    System.out.println("Puesto [" + i + "][" + j + "]: Vehículo "
                            + puestos[i][j].getVehiculoOcupante().getPlaca() +
    ...
    public static void validarInfo(Puesto[][] puestos, int fila, int columna) {
        ...
        if (fila >= 0 && fila < puestos.length && columna >= 0 && columna < puestos[0].length) {
            ...
            if (puesto.isEstaOcupado()) {
                ...
                System.out.println(
                        "Propietario del vehículo en el puesto [" + fila + "][" + columna + "]: " + propietario);
                System.out.println();
            } else {
